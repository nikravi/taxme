...
async function deployTaxMe(): Promise<void> {
  ...
  const qcCollectorTx = await taxMeContract.addTaxCollector(
    "qc",
    "0x5c04ce3918cfb2535E249e0dFe6955FEd5D831EE"
  )
  console.log("TaxMe adding qc collector", qcCollectorTx.hash)
  await qcCollectorTx.wait(1)
  const onCollectorTx = await taxMeContract.addTaxCollector(
    "on",
    "0xc6515783D05DfDF8fb2d4B6866929ba5493C06e0"
  )
  console.log("TaxMe adding on collector", onCollectorTx.hash)
  await onCollectorTx.wait(1)
  const gstCollectorTx = await taxMeContract.addTaxCollector(
    "gst",
    "0x8Eb858508F36553a72284Ad8e9f9F51A96bAE6Df"
  )
